...
$load = assessment_tool_should_load_wpemerge( $name, '0.17.0', '2.0.0' );

if ( ! $load ) {
	// An incompatible WP Emerge version is already loaded - stop further execution.
	// assessment_tool_should_load_wpemerge() will automatically add an admin notice.
	return;
}

define( 'ASSESSMENT_TOOL_PLUGIN_FILE', __FILE__ );
...
assessment_tool_declare_loaded_wpemerge( $name, 'theme', __FILE__ );

// Load helpers.
require_once __DIR__ . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'AssessmentTool.php';
...
\AssessmentTool::make()->bootstrap( require __DIR__ . DIRECTORY_SEPARATOR . 'app' . DIRECTORY_SEPARATOR . 'config.php' );
